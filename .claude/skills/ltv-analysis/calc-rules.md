...
## 9. Diagnóstico obrigatório antes de atribuir diferença a produto

Quando dois produtos ou sub-produtos do mesmo grupo apresentam `tx_recompra` com diferença ≥ 10 p.p. **ou** `ltv_medio` com diferença ≥ 25%, verificar os 3 confundidores abaixo **nesta ordem** antes de concluir que um produto fideliza mais:

| Confundidor | Verificação | Sinal de confirmação |
|---|---|---|
| **Safra** | Comparar `tx_recompra` em janela fixa de 365 dias | Diferença cai ou some na janela fixa |
| **Perfil** | Comparar distribuição de `renda`, `escolaridade`, `papel_cliente` entre os grupos | Distribuição de perfil muito diferente entre produtos |
| **Ticket** | Comparar `t1_medio` × `tx_recompra` | Produto mais caro tem tx_rcmp proporcionalmente menor |

**Regra de reportagem:** só declarar diferença intrínseca de produto se nenhum confundidor explicar. Caso contrário, reportar o confundidor como causa — nunca o produto.

---

## 10. Campos que variam por análise

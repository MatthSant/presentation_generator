-- Spec 005: o agente propõe entradas (evento 'sugestao' cai na triagem) e o seed é idempotente.

-- 1) activity.evento aceita 'sugestao'. SQLite não altera CHECK: recria a tabela.
CREATE TABLE activity_new (
  id               TEXT PRIMARY KEY,
  org_id           TEXT NOT NULL REFERENCES orgs(id),
  email            TEXT NOT NULL,
  evento           TEXT NOT NULL CHECK (evento IN ('geracao', 'aprofundamento', 'edicao', 'sugestao')),
  slug             TEXT NOT NULL,
...
-- 2) hash do conteúdo da versão (manifesto + arquivos + tarefas + regras, sem o exemplo):
--    o seed não publica versão igual à que já está no ar.
ALTER TABLE template_versions ADD COLUMN content_hash TEXT;

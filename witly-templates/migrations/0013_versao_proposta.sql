-- Versão proposta pelo agente: quando ele já corrigiu durante o trabalho, sobe o template
-- corrigido como RASCUNHO (nunca publicada) em vez de descrever o conserto em prosa e
-- fazer o editor reescrever à mão. Evento 'versao' na atividade, para entrar na triagem.
-- SQLite não altera CHECK: recria a tabela (mesmo padrão da 0008 e da 0010).
CREATE TABLE activity_new (
  id               TEXT PRIMARY KEY,
  org_id           TEXT NOT NULL REFERENCES orgs(id),
  email            TEXT NOT NULL,
  evento           TEXT NOT NULL CHECK (evento IN ('geracao', 'aprofundamento', 'edicao', 'sugestao', 'feedback', 'versao')),
  slug             TEXT NOT NULL,

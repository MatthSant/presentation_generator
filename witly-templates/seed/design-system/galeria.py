...
ELEMENTOS = [
    ...
    ('grafico-linha', 'graficos', 'Linha no tempo', 'Série diária; `largo=True` ocupa a linha inteira (12×6).',
     "s.grafico('line', 'CPL por dia', 'q-dia', x='dia', y='cpl', fmt='money', largo=True)"),
...
def gerar(eid, grupo, title, desc, call):
    ...
    return {'id': eid, 'grupo': grupo, 'title': title, 'desc': desc, 'call': call,
            ...
            'dataset': {n: {'dims': TABELAS[n][0], 'filters': [], 'rows': TABELAS[n][1]} for n in sorted(usados)}}

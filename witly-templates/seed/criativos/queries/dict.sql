-- Dicionário de links: field_ad_name -> URL do post (permalink) — OPCIONAL
-- Fonte: wtl_paidmedia → facebook_ads.ad_history (ad→creative) → facebook_ads.creative_history
-- Um field_ad_name pode repetir em vários ad_id (recriações): DISTINCT ON pega o mais
-- recente por _fivetran_synced. 1ª coluna = field_ad_name (chave); 2ª = a URL.
-- Sem filtro de lançamento: cobre a conta inteira (o join no kit é pelo nome).
...
WHERE pm.field_ad_name IS NOT NULL

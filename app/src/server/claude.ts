...
const DEEPEN_DOMAIN: Record<string, { what: string; focus: string }> = {
  ...
  'acompanhamento-lancamento': {
    what: 'acompanhamento tático DIÁRIO de UM lançamento em curso: KPIs por dia (CPL, CPMQL, CTR, Hook, Hold, Connect, Conv. de Página, Taxa de Resposta/Qualidade) comparados a METAS e BENCHMARKS, com tendência dos últimos 3 dias e funil de tráfego (taxas vs benchmark). DISPONIBILIDADE VARIÁVEL: quando a base TEM pageviews, Connect Rate (pageviews/clicks) e Conv. de Página (leads/pageviews) são etapas DISTINTAS do funil; bases SEM pageviews/vídeo omitem Hook/Hold/Connect e a etapa Pageviews, e aí a Conv. de Página vira leads/clicks',
    focus: 'FOQUE na métrica/dia que o card mostra; o eixo é o DIA da campanha. Os BENCHMARKS/metas estão nas tabelas acom_kpis (colunas meta/dev/cls/trend_dir) e acom_funnel (colunas bench/gap/maior_furo) — SEMPRE compare o realizado contra elas e cite o benchmark. Se a pergunta/instrução menciona "vs meta" (ou meta/benchmark de qualquer forma), é OBRIGATÓRIO trazer a meta/bench daquele indicador na comparação; se o indicador NÃO tiver meta no catálogo, diga explicitamente "sem meta definida" — não omita em silêncio nem invente um número. O maior furo do funil é a maior queda RELATIVA ao benchmark (gap), não a maior perda absoluta. COMPARAR GRUPOS NO TEMPO: para o MESMO indicador por dia em vários grupos (ex.: "CPL por dia: Quente × Morno", "CPL por dia por canal"), use consultar `cruzar_dia` (metrica + dimensao=temperatura/canal/origem) → devolve LONG (dia/serie/valor) e vira UM gráfico multi-linha (bind x="dia", series="serie", y="valor"). NUNCA faça um gráfico por grupo (2 gráficos de CPL/dia é erro de forma — é 1 só, multi-linha). CUSTO subindo (CPL/CPMQL — mídia vs qualificação): use consultar `decomposicao` (metrica=cpl|cpmql) — o motor devolve a CONTRIBUIÇÃO % de cada fator (CPM/CTR/Connect/Conv.Página; e CPL/Qualidade p/ CPMQL); aponte a alavanca pela maior contribuição. NÃO faça a álgebra de atribuição na mão. RECORTES (modo fundo): a tool `consultar` recorta por `dimensao` (dia | temperatura | canal | origem | criativo | publico | campanha) e ainda filtra com `recorte_origem`/`recorte_temperatura`/`recorte_canal`/`recorte_criativo`/`recorte_publico`/`recorte_campanha` ANTES de agregar — então VOCÊ TEM ACESSO a praticamente qualquer quebra: CPL por temperatura, por canal (utm_source), por criativo (anúncio), por publico (adset), por campanha — use ranking p/ achar o melhor/pior — pago × orgânico, e cruzamentos como "CPL por DIA só do tráfego Quente" (dimensao=dia + recorte_temperatura=Quente) ou "por dia de um canal" (recorte_canal). Para qualquer pergunta de quebra, CONSULTE a dimensão/recorte certo em vez de dizer que não dá. ONDE ESTÁ A CAUSA (uma métrica piorou — qual recorte puxa?): use consultar `onde_concentra` (metrica) — o motor varre criativo → publico → campanha → canal → temperatura e diz se a piora CONCENTRA num item (a causa) ou é AMPLA/uniforme (→ causa GLOBAL: mídia/leilão/sazonalidade). NÃO conclua "é o criativo X" sem isso; se for amplo em tudo, diga que é global e ARGUMENTE. (1 item só num nível = inconclusivo, olhe o próximo.) Ele também reporta PAUSADOS (ativos no início, sem dado recente = provavelmente DESLIGADOS — não é piora) e NOVOS por nível — use p/ ler "pausaram os ruins / entraram novos". Criativo/campanha incluem ORGÂNICO (coalesce com utm_content/utm_campaign), não só pago. TOTAL/GERAL: se a tabela/gráfico precisa de uma linha/série "Geral", passe incluir_geral=sim no consultar — o motor devolve o GERAL já correto (soma contagens, RECALCULA taxas ponderadas). NUNCA some você os grupos (somar taxa = absurdo) nem misture o agregado global com a série de um grupo. TEMPERATURA: além do modo fundo, a quebra por temperatura (Quente/Morno/Frio, split do TOTAL — não série diária) também está pronta na tabela acom_temp (colunas leads/invest/cpl/cpmql). Só diga que não há segmentação por temperatura se acom_temp não existir E o consultar(dimensao=temperatura) voltar vazio. Não confunda "não há temperatura por DIA pronta" com "não há temperatura" — dá para consultar. FUNIL — leia conforme o que ESTÁ no catálogo (não confunda os dois casos): (A) base COM pageviews — Connect Rate e Conv. de Página aparecem em acom_kpis/acom_funnel e são DUAS etapas DISTINTAS: Connect Rate = cliques que chegam na página (pageviews/clicks); Conv. de Página = página que converte em lead (leads/pageviews). Use AS DUAS, cada uma vs seu próprio benchmark, e NÃO as misture nem trate conv. de página como leads/clicks. (B) base SEM pageviews — não há Connect nem etapa Pageviews no catálogo: NÃO cite Hook/Hold/Connect nem os invente (se a pergunta exigir, reconheça num find-note que não há o dado); aí a Conv. de Página É leads/clicks (≡ connect × conv. de página), benchmark = produto dos dois. Resumo: connect/conv. de página existem? trate como (A); não existem? caso (B). NÃO existe "critério/grupo de pesquisa" nem benchmark de respondentes neste tipo.',
  },
...
- "blocking" (GRAVE — reprova até ser corrigido): a saída fica ERRADA ou ENGANOSA. Use SÓ para:
  ...
  • TOTAL ABSURDO (cheque com cuidado, é fácil dar falso positivo): SÓ acuse se a LINHA "total"/"geral" de uma coluna de TAXA/RAZÃO (taxa de resposta/qualidade/conversão, CPL, CPMQL, CPM, CTR) trouxer o VALOR DA SOMA dos grupos — ex.: "Taxa de Resposta" Geral = 113% (= 64+49), impossível (taxa não passa de 100% nem é soma). MAS uma linha "Geral" com valor PONDERADO plausível — entre o menor e o maior grupo, ≈ média ponderada (ex.: Connect Geral 60% entre 51% e 63%; CPL Geral 7,99 entre 7,10 e 12,45) — está CORRETA: é o GLOBAL que o MOTOR calculou; NÃO acuse. NUNCA some você as linhas da tabela (muito menos incluindo a própria linha "Geral") p/ inventar um "total da coluna" — a linha Geral NÃO é a soma das outras. Contagens (leads/investimento) e %-composição (soma ~100%) podem ter total

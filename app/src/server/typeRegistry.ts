      ...
      return {
        consultar: {
          funcoes: [
            ...
            { id: 'cruzar_dia', desc: 'UMA métrica por DIA × dimensão (escopo/canal/temperatura/criativo/publico/campanha) em formato LONG (dia/serie/valor). Vale p/ TAXA/CUSTO e p/ VOLUME (leads/vendas): use como UM gráfico multi-linha comparando grupos no tempo (bind x="dia", series="serie", y="valor"), p/ saturação/evolução (ex.: "CPL por dia por temperatura" = 1 gráfico). Para o split de UM dia específico (ex.: "leads por canal/criativo no dia de pico"), NÃO estime por proporção do período — use ranking/tabela com recorte_dia, que devolve o volume REAL daquele dia. Lembre: criativo = utm_content no orgânico.' },
          ...
          params: {
            ...
            recorte_publico: { desc: 'filtra a um público/adset específico (use um valor visto na dimensão publico)' },
            recorte_campanha: { desc: 'filtra a uma campanha específica (use um valor visto na dimensão campanha)' },
            recorte_dia: { desc: 'filtra a UM dia específico — rótulo DD/MM visto na série (ex.: "09/04") ou ISO YYYY-MM-DD. Habilita o breakdown REAL de um único dia: ex.: dimensao=criativo + recorte_dia="09/04" = leads por criativo/utm_content NAQUELE dia; dimensao=canal + recorte_dia = por canal. USE isto p/ o split de um dia de pico em vez de estimar pela proporção do período.' },

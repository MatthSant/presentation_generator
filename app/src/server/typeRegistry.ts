      ...
      return {
        consultar: {
          funcoes: [
            ...
            { id: 'impacto_receita', desc: 'PONTE DE FATURAMENTO: decompõe a variação de RECEITA (atual × baseline) na contribuição de cada ETAPA DO FUNIL, em % e em R$ — Faturamento = Volume(leads) × Taxa de Resposta × Qualificação × (Vendas÷MQL) × Ticket. Responde "quanto a queda de qualificação (ou de volume/ticket) custou de RECEITA". Vendas÷MQL é razão vendas/MQLs (não há vendas atribuídas a MQL no dado — não é conversão MQL→venda; pode passar de 1 no orgânico). param base=meta|historico|janela (default meta se houver metas); recorte_* p/ um segmento (ex.: só o Pago). A etapa de maior |R$| é a alavanca — o motor faz a álgebra.' },
          ],
          params: {

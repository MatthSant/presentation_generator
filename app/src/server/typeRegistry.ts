      ...
      return {
        consultar: {
          funcoes: [
            ...
            { id: 'impacto_receita', desc: 'PONTE DE FATURAMENTO: decompõe a variação de RECEITA (atual × baseline) na contribuição de cada ETAPA DO FUNIL, em % e em R$ — Faturamento = Volume(leads) × Taxa de Resposta × Qualificação × Fechamento(MQL→venda) × Ticket. Responde "quanto a queda de qualificação (ou de conversão/volume/ticket) custou de RECEITA". param base=meta|historico|janela (default meta se houver metas); recorte_* p/ um segmento (ex.: só o Pago). A etapa de maior |R$| é a alavanca — o motor faz a álgebra.' },
          ],
          params: {

...
export function registerGenerate(app: Express, ctx: Ctx): void {
  ...
  app.get('/api/:client/:slug/base-config', (req, res) => {
    ...
    const caps = { temperature: !!def?.supportsTemperature, campaignType: !!def?.supportsCampaignType,
                   goals: !!def?.supportsGoals, dict: !!def?.supportsDict, goalsUi: def?.goalsUi || null };
